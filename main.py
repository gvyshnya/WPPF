...
# Out-of-Sample Test Case Parameters
#--------------------------------------------------------

# I. Smoothening switch
# - Set do_smoothening to 0 if you do not want to smoothen the raw training data for Power Production
# - Set do_smoothening to 1, if you like to smoothen raw power production time series
do_smoothening = 0

# II. Smoothening Q-scale parameter
q = 4

# III. Date for the entire obervations split into training/validation set
validation_set_start_date = "2016-10-01 00:00:00"

# IV. kNN, phase 1 parameters - searching for closest match(-es) in pre-calculated fractal
#     dimension intervals (on 3h timeframe scale)
weights1 = [0.3,0.5,0.2]
k1 = 3

# V. kNN, phase 2 parameters - searching for closest match(-es) in power production
#    observations within the training subset
...
###############################################################
# Step 3: Main forecasting and out-of-sample validation flow
###############################################################

slicer = WPFDatasetSlicer.WPFDatasetSlicer(training_df)
slicer.split(validation_set_start_date)  # this will split training_df into training and validation parts
validation_df = slicer.validation_df

print ("Completed split of data into training and validation data sets, split date: ", validation_set_start_date)

# prepare validation df for modelling - separate predictors and the target variable
predictors_df = validation_df.filter(['Timestamp',
                'Temperature, C (observation)',
                'Wind speed, km/h (observation)',
                'Wind direction, degrees (observation)'],
                axis=1)
targets = validation_df['Produktion [kW]'].values

print("Prepared predictors and targets for modelling and validation ... ")

model = Modeller.Modeller(predictors_df, targets, slicer, k1, weights1, k2, weights2)

model.predict()
print ("Model prediction completed ...")

rmse = model.rmse()
print ("Model validation completed; RMSE on validation data set = ", rmse)

mape = model.mean_absolute_percentage_error()
print ("Model validation completed; MAPE on validation data set = ", mape)

predictions = model.predictions
print("Preparing the output ...")
print ("Model predictions = ", predictions)

# do output of the prediction and validation results
output_col_list = ['Timestamp', 'Produktion [kW]', 'Forecast [kW]']
output_df = pd.DataFrame(columns=output_col_list)
output_df['Timestamp'] = validation_df['Timestamp']
output_df['Produktion [kW]'] = validation_df['Produktion [kW]']
output_df['Forecast [kW]'] = predictions

output_df['MAPE'] = np.abs((output_df['Produktion [kW]'] -
                            output_df['Forecast [kW]']) / output_df['Produktion [kW]'] * 100)

# re-create the new test results output file
output_file = "testing_results.csv"
